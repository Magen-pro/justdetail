...
async function openPaymentModal(preset) {
  ...
  document.getElementById('payDate').value = toDateStr(new Date());
  ...
  if (preset && preset.type === 'one_time') {
    document.getElementById('paySource').value = 'one_time';
    document.getElementById('payOneTimeRow').style.display = 'block';
    document.getElementById('payOneTimeBooking').value = preset.id;
  } else if (preset && preset.type === 'subscription') {
    document.getElementById('paySource').value = 'subscription';
    document.getElementById('paySubscriptionRow').style.display = 'block';
    document.getElementById('paySubscription').value = preset.id;
...
async function populateOneTimeOptions() {
  ...
  const select = document.getElementById('payOneTimeBooking');
  select.innerHTML = (data || []).map(b =>
    `<option value="${b.id}">${b.customer_name} — ${b.service} (${formatDate(b.created_at)})</option>`
  ).join('') || '<option value="">No bookings found</option>';
}

async function autoFillSubscriptionAmount(subscriptionId) {
  const select = document.getElementById('paySubscription');
  const option = select.querySelector(`option[value="${subscriptionId}"]`);
  const planPrice = option ? Number(option.dataset.price) : 0;
  if (!planPrice) return;
...
async function populateSubscriptionOptions() {
  ...
  select.innerHTML = (data || []).map(s =>
    `<option value="${s.id}" data-price="${s.plans?.price || ''}">${s.clients?.full_name || 'Customer'} — ${s.plans?.tier_name || 'Plan'} (${s.vehicle_model || ''})</option>`
